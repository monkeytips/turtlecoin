...
# monkeytips Multi-installer
# a one line clone-and-compile for monkeytips:
#
#     ` $ curl -sL "https://raw.githubusercontent.com/monkeytips/monkeytips/master/scripts/multi_installer.sh" | bash
#
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        if [ -d "$PWD"/monkeytips ]; then
            read -r -p "${1:-monkeytips directory already exists. Overwrite? [y/N]} " response
            case "$response" in
                [yY][eE][sS|[yY])
                    _colorize red "Overwriting old monkeytips directory" && echo
                    rm -rf "$PWD"/monkeytips
                    ;;
                *)
                    _fail "monkeytips directory already exists. Aborting..."
                    ;;
            esac
        fi
        mkdir monkeytips
        git clone -b master -q https://github.com/monkeytips/monkeytips monkeytips   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd monkeytips
    fi
}

_build_monkeytips() {
    _note "Building monkeytips from source (this might take a while)..."
    if [ -d build ]; then
...
_configure_linux() {
    if [ "$(awk -F= '/^NAME/{print $2}' /etc/os-release)" = "\"Ubuntu\"" ]; then
        ...
        _fail "Your OS version isn't supported by this installer. Please consider adding support for your OS to the project ('https://github.com/monkeytips')"
    fi
}
...
_configure_os() {
    ...
    case "${_unameOut}" in
        ...
        Darwin*)
            ...
            ;;
        *)
            _fail "This installer only runs on OSX 10.10+ and Ubuntu 16.04+. Please consider adding support for your OS to the project ('https://github.com/monkeytips')"
            ;;
    esac
    _note "Operating system configuration completed. You're halfway there!"
}

_note "monkeytips Multi_Installer v1.0 (pepperoni)"
...
_note "Look in 'monkeytips/build/src/' for the executible binaries. See 'https://github.com/monkeytips/monkeytips' for more project support. Cowabunga!"
